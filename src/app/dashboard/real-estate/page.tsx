...
export default function RealEstatePage() {
	...
	return (
		<VStack gap="xl">
					...
					<DialogContent style={{ maxWidth: '42rem', maxHeight: '90vh', overflowY: 'auto' }}>
						...
						<form onSubmit={handleSubmit}>
							...
							{formError && (
								...
								>
									<Text size="sm" style={{ color: 'hsl(var(--destructive))' }}>{formError}</Text>
								</Box>
							)}
										...
										{memberShares.length > 0 && (
											<Text size="xs" color="muted" style={{ textAlign: 'right' }}>
												...
												{memberShares.reduce((sum, ms) => sum + ms.ownershipShare, 0) !== 100 && (
													<span style={{ color: 'hsl(var(--destructive))', marginLeft: '0.25rem' }}>(doit être 100%)</span>
												)}
											</Text>
										)}
									</VStack>
			...
			{/* Error state */}
			{isError && (
				<GlassCard padding="md" style={{ borderColor: 'hsl(var(--destructive) / 0.5)', backgroundColor: 'hsl(var(--destructive) / 0.05)' }}>
				...
				{isLoading ? (
					...
					<>
						...
						<StatCard
							...
							description={summary.totalValue > 0 ? `${((summary.totalLoansRemaining / summary.totalValue) * 100).toFixed(0)}% de la valeur` : '-'}
							icon={TrendingUp}
							variant="default"
						/>
						...
						<StatCard
							label="Équité"
							value={summary.totalValue > 0 ? `${((summary.totalEquity / summary.totalValue) * 100).toFixed(0)}%` : '-'}
			...
			{!isLoading && summary && summary.totalValue > 0 && (
				<GlassCard padding="lg">
					...
					<Progress value={(summary.totalLoansRemaining / summary.totalValue) * 100} style={{ height: '0.75rem' }} />
						...
						return (
							<GlassCard key={property.id} padding="lg" style={{ overflow: 'hidden' }}>
								...
								<HStack justify="between" align="start">
									...
									>
										...
										<Box style={{ minWidth: 0 }}>
											<Heading level={3} size="lg" weight="medium" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{property.name}</Heading>
											<HStack gap="xs" align="center" style={{ marginTop: '0.125rem' }}>
												<MapPin style={{ height: '0.75rem', width: '0.75rem', flexShrink: 0, color: 'hsl(var(--muted-foreground))' }} />
												<Text size="xs" color="muted" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{property.address}, {property.city}</Text>
											</HStack>
										</Box>
									</Link>
									<DropdownMenu>
								...
								<HStack gap="sm" style={{ flexWrap: 'wrap' }}>
									...
									{/* Value & Equity */}
									<Grid cols={2} gap="md">
										<Box style={{ borderRadius: '0.75rem', padding: '0.75rem', backgroundColor: 'hsl(var(--muted) / 0.3)' }}>
											<Text size="xs" color="muted">Valeur actuelle</Text>
											<Text size="xl" weight="semibold" style={{ fontVariantNumeric: 'tabular-nums', marginTop: '0.25rem' }}>
										...
										<Box style={{ borderRadius: '0.75rem', padding: '0.75rem', backgroundColor: 'hsl(var(--muted) / 0.3)' }}>
											<Text size="xs" color="muted">Équité</Text>
											<Text
												size="xl"
												weight="semibold"
												style={{
									...
									{/* Info Grid */}
									<Grid cols={3} gap="md" style={{ paddingTop: '0.5rem', borderTop: '1px solid hsl(var(--border) / 0.4)' }}>
										<Box style={{ textAlign: 'center' }}>
											<HStack gap="xs" align="center" justify="center" style={{ color: 'hsl(var(--muted-foreground))' }}>
												<Home style={{ height: '0.875rem', width: '0.875rem' }} />
												<Text size="xs">Surface</Text>
											</HStack>
											<Text weight="medium" style={{ marginTop: '0.25rem' }}>{property.surface} m²</Text>
										</Box>
										<Box style={{ textAlign: 'center' }}>
											<HStack gap="xs" align="center" justify="center" style={{ color: 'hsl(var(--muted-foreground))' }}>
											...
											<Text weight="medium" style={{ marginTop: '0.25rem' }}>{property.rooms || '-'}</Text>
										</Box>
										<Box style={{ textAlign: 'center' }}>
											<HStack gap="xs" align="center" justify="center" style={{ color: 'hsl(var(--muted-foreground))' }}>
												<Building2 style={{ height: '0.875rem', width: '0.875rem' }} />
											...
											<Text weight="medium" style={{ marginTop: '0.25rem' }}>{property.bedrooms || '-'}</Text>
										</Box>
									</Grid>

									{/* Rent Info if rental */}
									{isRentalProperty && property.rentAmount && (
										<Box style={{ borderRadius: '0.75rem', padding: '0.75rem', backgroundColor: 'oklch(0.55 0.15 145 / 0.1)' }}>
											<HStack justify="between" align="center">
												<VStack gap="none">
													<Text size="xs" style={{ color: 'oklch(0.55 0.15 145)' }}>Loyer mensuel</Text>
													<Text size="lg" weight="semibold" style={{ fontVariantNumeric: 'tabular-nums', color: 'oklch(0.55 0.15 145)' }}>
														{formatCurrency(property.rentAmount)}
													</Text>
												</VStack>
												{property.rentCharges && (
													<Box style={{ textAlign: 'right' }}>
														<Text size="xs" color="muted">Net de charges</Text>
