...
export class Bank extends Entity<BankProps> {
	...
	static create(props: { userId: UniqueId; name: string; template: BankTemplate }, id?: UniqueId): Result<Bank> {
	...
	static reconstitute(props: { userId: UniqueId; name: string; template: BankTemplate; createdAt: Date }, id: UniqueId): Result<Bank> {
		return Result.ok(
			new Bank(
				{
					userId: props.userId,
