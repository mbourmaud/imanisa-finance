<script lang="ts">
	...
	async function saveRule() {
		...
		try {
			...
			await invalidateAll();
			resetForm();
			successMessage = editingRule ? 'Règle modifiée avec succès' : 'Règle créée avec succès';
	...
	async function deleteRule(rule: RuleJSON) {
		if (!confirm(`Supprimer la règle "${rule.pattern}" ?`)) {
			return;
		}
	...
	async function recategorize() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error('Erreur lors de la recatégorisation');
			}

			const result = await response.json();
			successMessage = `Recatégorisation terminée: ${result.categorized} transactions catégorisées`;
...
<div class="rules-page">
		...
		<div class="header-left">
			...
			<div class="header-titles">
				...
				<span class="header-subtitle">
					{data.rules.length} règle{data.rules.length > 1 ? 's' : ''}
		...
		<div class="alert alert-success">
			...
			{#if showRecategorizePrompt}
				<button class="btn btn-sm btn-secondary" onclick={recategorize} disabled={isLoading}>
					Recatégoriser les transactions
				</button>
			{/if}
		</div>
	{/if}
		...
		<div class="form-card">
			...
			<form class="rule-form" onsubmit={(e) => { e.preventDefault(); saveRule(); }}>
				<div class="form-group">
					...
					<span class="form-hint">Expression régulière (insensible à la casse)</span>
				</div>

				<div class="form-group">
					<label for="category">Catégorie cible</label>
				...
				<div class="form-row">
					<div class="form-group">
						...
						<input
							...
							class="form-input"
						/>
						<span class="form-hint">Plus élevé = vérifié en premier</span>
					</div>
					...
					<div class="form-group">
						...
						<input
							...
							class="form-input"
						/>
						<span class="form-hint">Appliquer uniquement à cette source</span>
					</div>
				</div>
				...
				<div class="form-actions">
					...
					<button type="submit" class="btn btn-primary" disabled={isLoading}>
						{isLoading ? 'Enregistrement...' : editingRule ? 'Modifier' : 'Créer'}
					</button>
				</div>
			</form>
		</div>
	...
	{#if data.rules.length === 0}
		<div class="empty-state">
			...
			<h2>Aucune règle</h2>
			<p>Créez des règles pour catégoriser automatiquement vos transactions</p>
			<button class="btn btn-primary" onclick={openAddForm}>
				Créer une règle
			</button>
		</div>
			...
			{#each data.rules as rule, index}
				<div class="rule-card" class:inactive={!rule.is_active} style="--anim-delay: {index * 0.03}s">
					...
					<div class="rule-actions">
						<label class="toggle-switch" title={rule.is_active ? 'Désactiver' : 'Activer'}>
							<input
								type="checkbox"
				...
				<div class="modal-body">
					...
					{:else if testResults.matches.length === 0}
						<div class="empty-matches">
							...
							<span class="empty-hint">{testResults.total_tested} transactions analysées</span>
						</div>
					{:else}
						<div class="matches-summary">
							<span class="matches-count">{testResults.matches_count}</span>
							correspondance{testResults.matches_count > 1 ? 's' : ''} trouvée{testResults.matches_count > 1 ? 's' : ''}
							<span class="matches-hint">sur {testResults.total_tested} transactions récentes</span>
						</div>
