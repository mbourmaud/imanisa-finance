...
import { createCEPersoService } from '@infrastructure/scrapers/CaisseEpargneService';
...
export const POST: RequestHandler = async () => {
	const username = env.CE_PERSO_USERNAME;
	const password = env.CE_PERSO_PASSWORD;
	...
	if (!username || !password) {
		return json(
			{
				success: false,
				error: 'CE Perso non configuré. Définissez CE_PERSO_USERNAME et CE_PERSO_PASSWORD dans .env'
			},
			{ status: 400 }
		);
	}
